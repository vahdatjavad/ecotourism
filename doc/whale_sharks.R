...
# shark_taxon <- search_taxa("Rhincodon typus")
# occ_data <- galah_call() |>
#   galah_identify(shark_taxon) |>
...
# whale_sharks_2024 <- occ_data |>
#   filter(!is.na(decimalLatitude), !is.na(decimalLongitude), !is.na(eventDate)) |>
#   rename(lat = decimalLatitude, lon = decimalLongitude)
# 
# # Save data
# usethis::use_data(whale_sharks_2024)

## ----echo=FALSE---------------------------------------------------------------
load("../data/whale_sharks_2024.rda")
head(whale_sharks_2024)

## ----echo=FALSE, fig.width=6, fig.height=4------------------------------------
whale_sharks_2024$month <- format(whale_sharks_2024$eventDate, "%B")
whale_sharks_2024$month_num <- as.numeric(format(whale_sharks_2024$eventDate, "%m"))

ggplot(whale_sharks_2024, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Whale Shark Occurrences (2024)",
...
whale_sharks_2024$date <- as.Date(whale_sharks_2024$eventDate)

date_range <- range(whale_sharks_2024$date)
total_days <- as.numeric(diff(date_range))
obs_days <- n_distinct(whale_sharks_2024$date)
gap_days <- total_days - obs_days

daily_counts <- whale_sharks_2024 %>%
  count(date) %>%
...
# library(geosphere)
# 
# nearest_station_ids <- sapply(1:nrow(whale_sharks_2024), function(i) {
#   dists <- distHaversine(
#     cbind(weather_stations$lon, weather_stations$lat),
#     c(whale_sharks_2024$lon[i], whale_sharks_2024$lat[i])
#   )
#   weather_stations$stnid[which.min(dists)]
# })
# 
# whale_sharks_2024$weather_station_id <- nearest_station_ids
# whale_sharks_2024_ws <- whale_sharks_2024 %>%
#   left_join(weather_stations, by = c("weather_station_id" = "stnid"))
# 
# usethis::use_data(whale_sharks_2024_ws)

## ----eval=TRUE, include=TRUE, echo=FALSE--------------------------------------
load("../data/whale_sharks_2024_ws.rda")
head(whale_sharks_2024_ws, n = 3)

## ----echo=FALSE---------------------------------------------------------------
load("../data/whale_sharks_2024_ws.rda")
state_freq <- table(whale_sharks_2024_ws$state)
...
whale_sharks_2024_ws$month <- format(whale_sharks_2024_ws$eventDate, "%B")
whale_sharks_2024_ws$month_num <- as.numeric(format(whale_sharks_2024_ws$eventDate, "%m"))

ggplot(whale_sharks_2024_ws, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Whale Shark Occurrences (2024) by State",
    x = "Month", y = "Number of Records"
  ) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
whale_sharks_2024_ws$date <- as.Date(whale_sharks_2024_ws$eventDate)
whale_sharks_2024_ws <- whale_sharks_2024_ws %>%
  mutate(week = week(date),
         week_start = floor_date(date, "week"))

weekly_counts <- whale_sharks_2024_ws %>%
...
ggplot(weekly_counts, aes(x = week_start, y = n, group = state)) +
  ...
  facet_wrap(~state, scales = "free_y") +
  labs(
    title = "Weekly Occurrence Patterns of Whale Sharks (2024) with Trend Lines",
...
state_monthly <- whale_sharks_2024_ws %>%
...
ggplot(state_monthly, aes(x = month, y = percent, color = state, group = state)) +
  ...
  labs(title = "Seasonal Patterns of Whale Shark Occurrences by State",
...
ggplot() +
  ...
  labs(
    title = "Distribution of Whale Shark Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
...
ggplot() +
  ...
  labs(
    title = "Monthly Distribution of Whale Shark Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
