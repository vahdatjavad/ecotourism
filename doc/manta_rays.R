...
# manta_taxon <- search_taxa("Manta birostris")
# occ_data <- galah_call() |>
#   galah_identify(manta_taxon) |>
...
# manta_rays_2024 <- occ_data |>
#   filter(!is.na(decimalLatitude), !is.na(decimalLongitude), !is.na(eventDate)) |>
#   rename(lat = decimalLatitude, lon = decimalLongitude)
# 
# # Save data
# usethis::use_data(manta_rays_2024)

## ----echo=FALSE---------------------------------------------------------------
load("../data/manta_rays_2024.rda")
head(manta_rays_2024)

## ----echo=FALSE, fig.width=6, fig.height=4------------------------------------
manta_rays_2024$month <- format(manta_rays_2024$eventDate, "%B")
manta_rays_2024$month_num <- as.numeric(format(manta_rays_2024$eventDate, "%m"))

ggplot(manta_rays_2024, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Manta Ray Occurrences (2024)",
...
manta_rays_2024$date <- as.Date(manta_rays_2024$eventDate)

date_range <- range(manta_rays_2024$date)
total_days <- as.numeric(diff(date_range))
obs_days <- n_distinct(manta_rays_2024$date)
gap_days <- total_days - obs_days

daily_counts <- manta_rays_2024 %>%
  count(date) %>%
...
# library(geosphere)
# 
# nearest_station_ids <- sapply(1:nrow(manta_rays_2024), function(i) {
#   dists <- distHaversine(
#     cbind(weather_stations$lon, weather_stations$lat),
#     c(manta_rays_2024$lon[i], manta_rays_2024$lat[i])
#   )
#   weather_stations$stnid[which.min(dists)]
# })
# 
# manta_rays_2024$weather_station_id <- nearest_station_ids
# manta_rays_2024_ws <- manta_rays_2024 %>%
#   left_join(weather_stations, by = c("weather_station_id" = "stnid"))
# 
# usethis::use_data(manta_rays_2024_ws)

## ----eval=TRUE, include=TRUE, echo=FALSE--------------------------------------
load("../data/manta_rays_2024_ws.rda")
head(manta_rays_2024_ws, n = 3)

## ----echo=FALSE---------------------------------------------------------------
load("../data/manta_rays_2024_ws.rda")
state_freq <- table(manta_rays_2024_ws$state)
...
manta_rays_2024_ws$month <- format(manta_rays_2024_ws$eventDate, "%B")
manta_rays_2024_ws$month_num <- as.numeric(format(manta_rays_2024_ws$eventDate, "%m"))

ggplot(manta_rays_2024_ws, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Manta Ray Occurrences (2024) by State",
    x = "Month", y = "Number of Records"
  ) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
manta_rays_2024_ws$date <- as.Date(manta_rays_2024_ws$eventDate)
manta_rays_2024_ws <- manta_rays_2024_ws %>%
  mutate(week = week(date),
         week_start = floor_date(date, "week"))

weekly_counts <- manta_rays_2024_ws %>%
...
ggplot(weekly_counts, aes(x = week_start, y = n, group = state)) +
  ...
  facet_wrap(~state, scales = "free_y") +
  labs(
    title = "Weekly Occurrence Patterns of Manta Rays (2024) with Trend Lines",
...
state_monthly <- manta_rays_2024_ws %>%
...
ggplot(state_monthly, aes(x = month, y = percent, color = state, group = state)) +
  ...
  labs(title = "Seasonal Patterns of Manta Ray Occurrences by State",
...
ggplot() +
  ...
  labs(
    title = "Distribution of Manta Ray Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
...
ggplot() +
  ...
  labs(
    title = "Monthly Distribution of Manta Ray Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
