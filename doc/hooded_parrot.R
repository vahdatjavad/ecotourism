...
# parrot_taxon <- search_taxa("Hooded parrot")
# occ_data <- galah_call() |>
#   galah_identify(parrot_taxon) |>
...
# hooded_parrot_2024 <- occ_data |>
#   filter(!is.na(decimalLatitude), !is.na(decimalLongitude), !is.na(eventDate)) |>
#   rename(lat = decimalLatitude, lon = decimalLongitude)
# 
# # Save data
# usethis::use_data(hooded_parrot_2024)

## ----echo=FALSE---------------------------------------------------------------
load("../data/hooded_parrot_2024.rda")
head(hooded_parrot_2024)

## ----echo=FALSE, fig.width=6, fig.height=4------------------------------------
hooded_parrot_2024$month <- format(hooded_parrot_2024$eventDate, "%B")
hooded_parrot_2024$month_num <- as.numeric(format(hooded_parrot_2024$eventDate, "%m"))

ggplot(hooded_parrot_2024, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Hooded Parrot Occurrences (2024)",
...
load("../data/hooded_parrot_2024.rda")
hooded_parrot_2024$date <- as.Date(hooded_parrot_2024$eventDate)

date_range <- range(hooded_parrot_2024$date)
total_days <- as.numeric(diff(date_range))
obs_days <- n_distinct(hooded_parrot_2024$date)
gap_days <- total_days - obs_days

daily_counts <- hooded_parrot_2024 %>%
  count(date) %>%
...
# library(geosphere)
# 
# nearest_station_ids <- sapply(1:nrow(hooded_parrot_2024), function(i) {
#   dists <- distHaversine(
#     cbind(weather_stations$lon, weather_stations$lat),
#     c(hooded_parrot_2024$lon[i], hooded_parrot_2024$lat[i])
#   )
#   weather_stations$stnid[which.min(dists)]
# })
# 
# hooded_parrot_2024$weather_station_id <- nearest_station_ids
# hooded_parrot_2024_ws <- hooded_parrot_2024 %>%
#   left_join(weather_stations, by = c("weather_station_id" = "stnid"))
# 
# usethis::use_data(hooded_parrot_2024_ws)

## ----eval=TRUE, include=TRUE, echo=FALSE--------------------------------------
load("../data/hooded_parrot_2024_ws.rda")
head(hooded_parrot_2024_ws, n = 3)

## ----echo=FALSE---------------------------------------------------------------
load("../data/hooded_parrot_2024_ws.rda")
state_freq <- table(hooded_parrot_2024_ws$state)
...
hooded_parrot_2024_ws$month <- format(hooded_parrot_2024_ws$eventDate, "%B")
hooded_parrot_2024_ws$month_num <- as.numeric(format(hooded_parrot_2024_ws$eventDate, "%m"))

ggplot(hooded_parrot_2024_ws, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Hooded Parrot Occurrences (2024) by State",
    x = "Month", y = "Number of Records"
  ) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
hooded_parrot_2024_ws$date <- as.Date(hooded_parrot_2024_ws$eventDate)
hooded_parrot_2024_ws <- hooded_parrot_2024_ws %>%
  mutate(week = week(date),
         week_start = floor_date(date, "week"))

weekly_counts <- hooded_parrot_2024_ws %>%
...
ggplot(weekly_counts, aes(x = week_start, y = n, group = state)) +
  ...
  facet_wrap(~state, scales = "free_y") +
  labs(
    title = "Weekly Occurrence Patterns of Hooded Parrot (2024) with Trend Lines",
...
state_monthly <- hooded_parrot_2024_ws %>%
...
ggplot(state_monthly, aes(x = month, y = percent, color = state, group = state)) +
  ...
  labs(title = "Seasonal Patterns of Hooded Parrot Occurrences by State",
...
ggplot() +
  ...
  labs(
    title = "Distribution of Hooded Parrot Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
...
ggplot() +
  ...
  labs(
    title = "Monthly Distribution of Hooded Parrot Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
