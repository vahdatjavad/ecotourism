...
# koala_taxon <- search_taxa("Koala")
# occ_data <- galah_call() |>
#   galah_identify(koala_taxon) |>
...
# 
# # Save data
# usethis::use_data(koala_2024)

## ----echo=FALSE---------------------------------------------------------------
load("../data/koala_2024.rda")
head(koala_2024)

## ----echo=FALSE, fig.width=6, fig.height=4------------------------------------
koala_2024$month <- format(koala_2024$eventDate, "%B")
koala_2024$month_num <- as.numeric(format(koala_2024$eventDate, "%m"))

ggplot(koala_2024, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Koala Occurrences (2024)",
...
koala_2024$date <- as.Date(koala_2024$eventDate)

date_range <- range(koala_2024$date)
total_days <- as.numeric(diff(date_range))
obs_days <- n_distinct(koala_2024$date)
gap_days <- total_days - obs_days

daily_counts <- koala_2024 %>%
  count(date) %>%
...
# library(geosphere)
# 
# nearest_station_ids <- sapply(1:nrow(koala_2024), function(i) {
#   dists <- distHaversine(
#     cbind(weather_stations$lon, weather_stations$lat),
#     c(koala_2024$lon[i], koala_2024$lat[i])
#   )
#   weather_stations$stnid[which.min(dists)]
# })
# 
# koala_2024$weather_station_id <- nearest_station_ids
# koala_2024_ws <- koala_2024 %>%
#   left_join(weather_stations, by = c("weather_station_id" = "stnid"))
# 
# usethis::use_data(koala_2024_ws)

## ----eval=TRUE, include=TRUE, echo=FALSE--------------------------------------
load("../data/koala_2024_ws.rda")
head(koala_2024_ws, n = 3)

## ----echo=FALSE---------------------------------------------------------------
load("../data/koala_2024_ws.rda")
state_freq <- table(koala_2024_ws$state)
...
koala_2024_ws$month <- format(koala_2024_ws$eventDate, "%B")
koala_2024_ws$month_num <- as.numeric(format(koala_2024_ws$eventDate, "%m"))

ggplot(koala_2024_ws, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Koala Occurrences (2024) by State",
    x = "Month", y = "Number of Records"
  ) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
koala_2024_ws$date <- as.Date(koala_2024_ws$eventDate)
koala_2024_ws <- koala_2024_ws %>%
  mutate(week = week(date),
         week_start = floor_date(date, "week"))

weekly_counts <- koala_2024_ws %>%
...
ggplot(weekly_counts, aes(x = week_start, y = n, group = state)) +
  ...
  facet_wrap(~state, scales = "free_y") +
  labs(
    title = "Weekly Occurrence Patterns of Koalas (2024) with Trend Lines",
...
ggplot(state_monthly, aes(x = month, y = percent, color = state, group = state)) +
  ...
  labs(title = "Seasonal Patterns of Koala Occurrences by State",
...
ggplot() +
  ...
  labs(
    title = "Distribution of Koala Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
...
ggplot() +
  ...
  labs(
    title = "Monthly Distribution of Koala Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
