...
# glowworms_2024 <- occ_data |>
#   filter(!is.na(decimalLatitude), !is.na(decimalLongitude), !is.na(eventDate)) |>
#   rename(lat = decimalLatitude, lon = decimalLongitude)
# 
# # Save data
# usethis::use_data(glowworms_2024)

## ----echo=FALSE---------------------------------------------------------------
load("../data/glowworms_2024.rda")
head(glowworms_2024)

## ----echo=FALSE, fig.width=6, fig.height=4------------------------------------
glowworms_2024$month <- format(glowworms_2024$eventDate, "%B")
glowworms_2024$month_num <- as.numeric(format(glowworms_2024$eventDate, "%m"))

ggplot(glowworms_2024, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Glowworm Occurrences (2024)",
...
glowworms_2024$date <- as.Date(glowworms_2024$eventDate)

date_range <- range(glowworms_2024$date)
total_days <- as.numeric(diff(date_range))
obs_days <- n_distinct(glowworms_2024$date)
gap_days <- total_days - obs_days

daily_counts <- glowworms_2024 %>%
  count(date) %>%
...
# library(geosphere)
# 
# nearest_station_ids <- sapply(1:nrow(glowworms_2024), function(i) {
#   dists <- distHaversine(
#     cbind(weather_stations$lon, weather_stations$lat),
#     c(glowworms_2024$lon[i], glowworms_2024$lat[i])
#   )
#   weather_stations$stnid[which.min(dists)]
# })
# 
# glowworms_2024$weather_station_id <- nearest_station_ids
# glowworms_2024_ws <- glowworms_2024 %>%
#   left_join(weather_stations, by = c("weather_station_id" = "stnid"))
# 
# usethis::use_data(glowworms_2024_ws)

## ----eval=TRUE, include=TRUE, echo=FALSE--------------------------------------
load("../data/glowworms_2024_ws.rda")
head(glowworms_2024_ws, n = 3)

## ----echo=FALSE---------------------------------------------------------------
load("../data/glowworms_2024_ws.rda")
state_freq <- table(glowworms_2024_ws$state)
...
glowworms_2024_ws$month <- format(glowworms_2024_ws$eventDate, "%B")
glowworms_2024_ws$month_num <- as.numeric(format(glowworms_2024_ws$eventDate, "%m"))

ggplot(glowworms_2024_ws, aes(x = factor(month, levels = month.name))) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Monthly Distribution of Glowworm Occurrences (2024) by State",
    x = "Month", y = "Number of Records"
  ) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
glowworms_2024_ws$date <- as.Date(glowworms_2024_ws$eventDate)
glowworms_2024_ws <- glowworms_2024_ws %>%
  mutate(week = week(date),
         week_start = floor_date(date, "week"))

weekly_counts <- glowworms_2024_ws %>%
...
ggplot(weekly_counts, aes(x = week_start, y = n, group = state)) +
  ...
  facet_wrap(~state, scales = "free_y") +
  labs(
    title = "Weekly Occurrence Patterns of Glowworms (2024) with Trend Lines",
...
state_monthly <- glowworms_2024_ws %>%
...
ggplot() +
  ...
  labs(
    title = "Distribution of Glowworm Occurrences Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
...
ggplot() +
  ...
  labs(
    title = "Monthly Distribution of Glowworm Occurrences Sightings in Australia (2024)",
    x = "Longitude",
    y = "Latitude",
    color = "State"
  )
